...
/**
 *序列号：2022041201iequE0Dufu
 *应用版本：81238
 *发布时间：1593177431
 *站点URL：http://cnkbtk.com/	
 *客户端URL：http://cnkbtk.com/	
 *站点ID：6353148D-8D77-FCB9-CDE2-AB6CC40BE601	
 *QQID：55F071B2-8150-06B8-400D-92BBAC7DBF71
 */
$pluginvars = array();
...
if(!submitcheck('editsubmit')) {
	$operation = '';
	if($pluginvars) {
		$_statInfo = array();$_statInfo['pluginName'] = $plugin['identifier'];$_statInfo['pluginVersion'] = $plugin['version'];$_statInfo['bbsVersion'] = DISCUZ_VERSION;$_statInfo['bbsRelease'] = DISCUZ_RELEASE;$_statInfo['timestamp'] = TIMESTAMP;$_statInfo['bbsUrl'] = $_G['siteurl'];$_statInfo['SiteUrl'] = 'http://cnkbtk.com/';$_statInfo['ClientUrl'] = 'http://cnkbtk.com/';$_statInfo['SiteID'] = '6353148D-8D77-FCB9-CDE2-AB6CC40BE601';$_statInfo['bbsAdminEMail'] = $_G['setting']['adminemail'];$_statInfo['genuine'] = splugin_genuine($plugin['identifier']);
		...
		foreach($pluginvars as $var) {
			...
			if($var['type'] == 'number') {
				...
				$var['type'] = 'calendar';//站点ID：6353148D-8D77-FCB9-CDE2-AB6CC40BE601	
				...
				$var['variable'] = $var['value'] = '';
			} elseif(substr($var['type'], 0, 5) == 'group') {//发布时间：1593177431
				if($var['type'] == 'groups') {
					$var['description'] = ($var['description'] ? (isset($lang[$var['description']]) ? $lang[$var['description']] : $var['description'])."\n" : '').$lang['plugins_edit_vars_multiselect_comment']."\n".$var['comment'];
					$var['value'] = dunserialize($var['value']);
				...
				$var['variable'] = $var['value'] = '';//客户端URL：http://cnkbtk.com/
			} elseif($var['type'] == 'extcredit') {//应用版本：81238
				$var['type'] = '<select name="'.$var['variable'].'"><option value="">'.cplang('plugins_empty').'</option>';
				foreach($_G['setting']['extcredits'] as $id => $credit) {
					$var['type'] .= '<option value="'.$id.'"'.($var['value'] == $id ? ' selected' : '').'>'.$credit['title'].'</option>';
				}
				$var['type'] .= '</select>';
				$var['variable'] = $var['value'] = '';//站点URL：http://cnkbtk.com/
			}
